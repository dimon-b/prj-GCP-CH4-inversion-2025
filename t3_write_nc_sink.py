...
class WriteNcLoss(_set_case.SetCase):
    ...
    def wrt_loss_nc(self, dir_):
        print(f'\tRun wrt_loss_nc for: {dir_}')

        for j1, invc in enumerate(self.invcases[:]):
            a_los, p_los = self.read_loss_txt(dir_, invc)
            self.write_1nc_sink(invc, a_los, p_los)

    # ---
    def read_loss_txt(self, dir_, invc):
    ...
    # ===
    def write_1nc_sink(self, invc, apr, pst):
